...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const middle = [a, b].sort((c, d) => c - d).join(', ');
...
function isCreditCardNumber(ccn) {
  ...
  const reverseCcn = ccnStr.split('').reverse().join('');
  let sum = 0;

  for (let i = 0; i < reverseCcn.length; i += 1) {
    const num = parseInt(reverseCcn[i], 10);
    if (i % 2 !== 0) {
      const double = num * 2;
      if (double > 9) {
        sum += (double - 9);
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < minLength; i += 1) {
    const segments = pathArrays.map((arr) => arr[i]);
...
function getMatrixProduct(m1, m2) {
  ...
  const columnsCountM1 = m1[0].length;
  const columnsCountM2 = m2[0].length;

  for (let i = 0; i < rowsCountM1; i += 1) {
    result[i] = [];
    for (let j = 0; j < columnsCountM2; j += 1) {
      let sum = 0;
      for (let k = 0; k < columnsCountM1; k += 1) {
        sum += m1[i][k] * m2[k][j];
      }
      result[i][j] = sum;
    }
